...
def set_seed(seed: int = 42):
    ...
    torch.backends.cudnn.benchmark = True # 开启 cudnn 自动寻优加速

class AdvancedEdgeLoss(nn.Module):
    """BIPED 专用的纹理抑制焦点损失 (Texture-Suppression Focal Loss)"""
    ...
    def _focal_bce(self, preds, targets):
        ...
        # Focal 权重：大幅度惩罚把背景纹理识别为边缘的 False Positives
...
def train(args):
    ...
    print(f"🔥 启动 BIPED 极限 SOTA 训练 (目标 ODS: 0.90+) | Device: {device}")
    
    Path(args.ckpt_dir).mkdir(parents=True, exist_ok=True)
    train_loader = DataLoader(BIPEDDataset(args.data_root, True), batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lr", type=float, default=2e-4) # 稍微调高学习率配合 Focal Loss
